...
TEST(tests_inverse_matrix, correct_arg) {
  ...
  S21Matrix answer(3, 3,
                   {0.0133333, -0.04, 0.04, 0.177778, -0.2, -0.0222222,
                    -0.213333, 0.306667, 0.0266667});
...
TEST(tests_inverse_matrix, correct_arg_8) {
  ...
  S21Matrix answer(3, 3, {1, -1, 1, -38, 41, -34, 27, -29, 24});
  S21Matrix result = matrix.InverseMatrix();
  EXPECT_TRUE(result.EqMatrix(answer));
  EXPECT_TRUE(answer.EqMatrix(result));
}
...
TEST(tests_inverse_matrix, correct_arg_10) {
  S21Matrix matrix(3, 3, {-1, -10, 1, 1, 10, 1, 1, 1, 1});
  S21Matrix answer(
      3, 3,
